            ...
            <div id="right" class="flex flex-col gap-y-4">
              ...
              <div class="flex flex-col gap-y-2">
                ...
                <div class="grid grid-cols-2 gap-x-4">
                  ...
                  <div class="flex flex-col gap-y-2">
                    <input id="starttime" type="time" v-model="eventStartTime" placeholder="" class="" :class="inputClass" />
...
export default {
  ...
  components: {
    ...
    submitForm() {
      ...
      if (checkForm) {
        ...
        const data = {
          ...
          eventCover: this.eventCover.name,
    ...
    async editEvent(data) {
      ...
      formData.append("file", this.eventCover, this.eventCover.name);
    ...
    async fetchContact() {
      const response = await fetch(`${this.host}/eventcontact/${this.$route.params.id}`, {
        method: "GET"
      })
      const oldContact = await response.json()
      return oldContact
    },
    ...
    async assignOldData(oldEvent, oldContact) {
      ...
      this.eventDescription = await oldEvent.eventLongDescription
      this.eventShortDesc = await oldEvent.eventShortDescription
      this.eventCost = await oldEvent.eventCost
      this.eventLocation = await oldEvent.eventLocation
      this.eventType = await oldEvent.eventType
      this.eventNumberOfPeople = await oldEvent.eventNumberOfPeople
      this.eventYear = await oldEvent.eventYear
      this.eventStartRegis = await oldEvent.eventStartRegis
      this.eventEndRegis = await oldEvent.eventEndRegis
      this.eventStartDate = await oldEvent.eventStartDate
      this.eventStartTime = await oldEvent.eventStartTime
      this.eventEndDate = await oldEvent.eventEndDate
      this.eventEndTime = await oldEvent.eventEndTime
      this.eventNote = await oldEvent.eventNote
      ...
      this.crole1 = await oldContact[0].contactRole
